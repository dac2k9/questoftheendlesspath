...
The hard-won rule: **an event's global `EventStatus` (Pending / Active /
Dismissed / Completed) is for content authoring and the initial trigger
gate ONLY. Every per-player decision reads per-player state, never the
global status.** Breaking this is the single most common source of
multiplayer bugs in this codebase (one player's dialog freezing
...
- **Combat** — `shared_combat`, keyed by a **session key**, not the bare
  ...
  event_id so all participants tick one shared session. This is why two
  players can solo the same boss without clobbering each other's
  `CombatState`. Victory/retreat routing reads `cs.event_id` from the
  state — NEVER parse the map key (it may carry a player suffix). The
  helpers: `start_combat(.., coop_players: &[String])` returns the key;
...
flag, for mid-fight joins and richer co-op. The current key split is the
minimum that makes solo correct without regressing co-op.
...
- `poi_at()` is an **exact tile match** (`p.x == x && p.y == y`) — a
  trigger's `at_poi` fires only when the player stands ON the POI tile,
  not adjacent. (`pois_near(x, y, radius)` is the radius-based variant,
  used for the trigger context's `nearby_poi_ids`.)
...
- **Combat is one-player-per-entity.** `check_contacts` skips an
  entity entirely when a live combat session already references its
  event_id (`combat::event_combat_active`, value-based — see the
  combat-session note below), and breaks after binding the first
  matching player on the tile. Without these guards two players
  standing on the same tile ping-ponged combat-state inserts each tick.
